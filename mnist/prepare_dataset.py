...
THIS_DIR = os.path.dirname(__file__)          # e.g., /path/to/mnist
TOP_DIR = os.path.dirname(THIS_DIR)
sys.path.append(TOP_DIR)

from rbfkan_utils.utils.dataset import group

DATASET_NAME = 'mnist'                       # single place to change dataset
...
def get_dataset_info() -> Dict:
    if DATASET_NAME == 'mnist':
        return {
            'num_classes': 10,
            'input_shape': (28, 28, 1),   # grayscale, channel last
            'task': 'multiclass',
        }
    else:
...
def _compute_stats_from_train() -> Tuple[np.ndarray, np.ndarray]:
    ...
    # data shape: (N, H, W, C) with C=1 for grayscale
    mean = data.mean(axis=(0, 1, 2))   # shape (C,)
...
if __name__ == "__main__":
    ...
    print(f"Mean (grayscale): {mean}, Std (grayscale): {std}")
